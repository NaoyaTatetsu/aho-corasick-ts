...
| `wordBoundary` | `'unicode'` | What `wholeWords` treats as a word character: `'unicode'` or `'ascii'`. Only valid alongside `wholeWords: true` |
...
| `wordBoundary` is set without `wholeWords: true` | `RangeError` |
