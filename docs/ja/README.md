...
Node.js 18以降が必要です（リリースごとに18.0.0から検証しています）。ESMとCommonJSの両方を、それぞれの型定義付きで同梱しているため、`import`と`require`のどちらでも読み込めます（Node 18を含む）。同一プロセスで両方を読み込むとクラスの実体が2つになりますが、影響があるのは実体同士を比較する場合だけです。実行時依存はゼロで、Node固有のAPIも使っていないため、ブラウザのESMとしても利用できます（ブラウザでの性能検証は未実施です）。
