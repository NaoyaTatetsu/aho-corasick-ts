...
export class AhoCorasick {
    ...
    constructor(patterns, options = {}) {
        ...
        if (!Number.isSafeInteger(budget) || budget < 0)
            throw new RangeError('maxDenseBytes must be a non-negative safe integer');
        ...
        for (let id = 0; id < dictionary.length; id++) {
            ...
            if (typeof pattern !== 'string')
                throw new TypeError('Patterns must be strings');
            if (pattern.length === 0)
                throw new RangeError('Empty patterns are not supported');
            let state = 0;
        ...
        if (bytes <= budget && size * width <= 0xffffffff) {
            ...
            for (const [code, child] of edges[0])
                table[alphabet[code]] = child;
        ...
        this.counts = counts;
        this.stats = Object.freeze({ states: size, alphabetSize: symbols, backend: table ? 'dense' : 'sparse', transitionBytes: table?.byteLength ?? 0 });
    }
    ...
    findAll(text) {
        ...
        const matches = this.manyOutputs ? new Array(this.count(text)) : [];
        let written = 0;
        let state = 0;
        for (let i = begin; i < text.length; i++) {
            state = table[state * width + alphabet[text.charCodeAt(i)]];
    ...
    forEachSmall(text, callback) {
        ...
        while (true) {
            ...
            if (best < 0)
                return;
            if (callback(best, bestStart, bestEnd) === false)
                return;
    ...
    forEach(text, callback) {
        if (this.smallDictionary)
            return this.forEachSmall(text, callback);
        const begin = this.scanStart(text);
        if (begin < 0)
            return;
        const { table, alphabet, width, outputs, outputLink, patterns } = this;
        ...
        if (table) {
            for (let i = begin; i < text.length; i++) {
                ...
                for (let output = state; output !== 0; output = outputLink[output]) {
                    ...
                    for (let j = 0; j < ids.length; j++) {
                        const id = ids[j];
                        if (callback(id, i + 1 - patterns[id].length, i + 1) === false)
                            return;
                    }
                }
            }
