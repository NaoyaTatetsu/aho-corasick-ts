...
const adapters = [
  ...
  ...[
    ['ahocorasick', Legacy],
    ['modern-ahocorasick', Modern],
  ].map(([name, C]) => ({
    name,
    build: p => new C(p),
    find: (a, t) => a.search(t),
    normalize: (rows, p) => rows.flatMap(([end, words]) => words.map(w => ({ patternIndex: p.indexOf(w), start: end + 1 - w.length, end: end + 1 }))),
    ...(name === 'modern-ahocorasick' ? { test: (a, t) => a.match(t) } : {}),
  })),
  ...[
    ['@monyone/aho-corasick', Monyone],
    ['@monyone/aho-corasick/fast', Fast],
  ].map(([name, C]) => ({
    name,
    build: p => new C(p),
    find: (a, t) => a.matchInText(t),
    normalize: (rows, p) => rows.map(x => ({ patternIndex: p.indexOf(x.keyword), start: x.begin, end: x.end })),
    test: (a, t) => a.hasKeywordInText(t),
  })),
];
...
try {
  ...
  adapters.push({
    ...
    build: p => new Native(p),
    find: (a, t) => a.findOverlappingIter(t),
    normalize: rows => rows.map(x => ({ patternIndex: x.pattern, start: x.start, end: x.end })),
    test: (a, t) => a.isMatch(t),
  });
} catch (e) {
  skipped.push({ name: '@stll/aho-corasick', reason: summarizeFailure(e) });
}
let seed = 42;
const random = n => {
...
const scenarios = [
  ...
  {
    name: 'large-alphabet',
    patterns: Array.from({ length: 2000 }, (_, i) => `${String.fromCharCode(1000 + i)}終`),
    text: Array.from({ length: 10000 }, (_, i) => `${String.fromCharCode(1000 + (i % 2000))}終`).join(''),
  },
];
...
function canonical(rows) {
  return rows.sort((a, b) => a.end - b.end || a.start - b.start || a.patternIndex - b.patternIndex);
}
function oracle(patterns, text) {
  ...
  patterns.forEach((p, patternIndex) => {
    ...
    while (start !== -1) {
      rows.push({ patternIndex, start, end: start + p.length });
      start = text.indexOf(p, start + 1);
    }
  });
...
if (!worker) {
  for (const scenario of scenarios)
    for (const adapter of adapters.filter(a => (process.argv.includes('--compare') ? ['local', 'before'].includes(a.name) : a.name !== 'before'))) {
      const child = spawnSync(
        process.execPath,
        [`--max-old-space-size=${HEAP_LIMIT_MB}`, fileURLToPath(import.meta.url), '--worker', scenario.name, adapter.name, ...(process.argv.includes('--compare') ? ['--compare'] : [])],
...
for (const scenario of scenarios) {
  ...
  for (const adapter of adapters) {
    ...
    const row = {
      ...
      build: measure(build),
      findAll: measure(() => adapter.find(ac, text)),
    };
    if (adapter.count) {
      assert.equal(adapter.count(ac, text), expected.length);
      row.count = measure(() => adapter.count(ac, text));
    }
    if (adapter.test) {
      assert.equal(adapter.test(ac, text), expected.length > 0);
      row.test = measure(() => adapter.test(ac, text));
    }
...
if (!worker) {
  const versions = Object.fromEntries(
    ['typescript', 'ahocorasick', 'modern-ahocorasick', '@monyone/aho-corasick', '@stll/aho-corasick'].map(name => [
      name,
  ...
  const report = {
    ...
    versions,
    methodology:
      '7 samples of >=35ms after >=60ms warmup; median/min/max milliseconds per call. Build and native result allocation are included in their respective operations. Normalization and correctness verification are outside timing. count has no equivalent in other tested APIs and is not an apples-to-apples findAll comparison. Synthetic fixtures only.',
    skipped,
    results,
  };
