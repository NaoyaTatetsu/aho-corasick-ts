...
function foldText(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (code >= 0xd800 && code <= 0xdfff) { folded += text[i]; continue; }
...
function selectOracle(patterns, text, { matchKind = 'all', caseInsensitive = false, wholeWords = false, wordBoundary = 'unicode' } = {}) {
  ...
  const byStart = [...matches].sort((a, b) =>
    a.start - b.start || (preferLongest ? (b.end - b.start) - (a.end - a.start) : 0) || a.patternIndex - b.patternIndex);
  const selected = [];
  let cursor = 0;
...
function assertAgrees(patterns, text, options) {
  for (const maxDenseBytes of [0, 64 * 1024 * 1024]) {
    ...
    if (expected.length > 1) {
      const partial = [];
      ac.forEach(text, (patternIndex, start, end) => { partial.push({ patternIndex, start, end }); return partial.length < 2; });
      assert.deepEqual(partial, expected.slice(0, 2), label);
    }
  }
}
const OPTION_MATRIX = [
  {}, { matchKind: 'leftmost-first' }, { matchKind: 'leftmost-longest' },
  { caseInsensitive: true }, { wholeWords: true }, { wholeWords: true, wordBoundary: 'ascii' },
...
test('leftmost kinds break ties by dictionary order and by length', () => {
  assert.deepEqual(new AhoCorasick(['ab', 'abc'], { matchKind: 'leftmost-first' }).findAll('abc'),
    [{ patternIndex: 0, start: 0, end: 2 }]);
  assert.deepEqual(new AhoCorasick(['abc', 'ab'], { matchKind: 'leftmost-first' }).findAll('abc'),
    [{ patternIndex: 0, start: 0, end: 3 }]);
  assert.deepEqual(new AhoCorasick(['ab', 'abc'], { matchKind: 'leftmost-longest' }).findAll('abc'),
    [{ patternIndex: 1, start: 0, end: 3 }]);
  ...
  for (const matchKind of ['leftmost-first', 'leftmost-longest']) {
    assert.deepEqual(new AhoCorasick(['a', 'a'], { matchKind }).findAll('aa'),
      [{ patternIndex: 0, start: 0, end: 1 }, { patternIndex: 0, start: 1, end: 2 }]);
  }
});
...
test('word boundaries respect code points, marks and the ascii mode', () => {
  ...
  assert.deepEqual(unicode.findAll('cat cats _cat 猫cat猫 (cat)').map(m => m.start), [0, 21]);
  const ascii = new AhoCorasick(['cat'], { wholeWords: true, wordBoundary: 'ascii' });
  assert.deepEqual(ascii.findAll('猫cat猫 cat').map(m => m.start), [1, 6]);
...
test('replace covers strings, per-pattern arrays, functions and validation', () => {
  ...
  assert.equal(ac.replace('ushers his', (id, start, end) => `[${id}:${start}-${end}]`), 'u[1:1-4]rs [2:7-10]');
  ...
  assert.throws(() => ac.replace('he', () => { throw new Error('replacer'); }), /replacer/);
});
test('new options reject unknown values and stay reentrant', () => {
  ...
  assert.deepEqual(seen, [[1, 0, 2], [0, 3, 4]]);
});
test('leftmost candidates settle on the path bound, not on the longest pattern', () => {
  ...
  assert.deepEqual(new AhoCorasick(['abc', 'ab'], { matchKind: 'leftmost-first' }).findAll('abcabc'),
    [{ patternIndex: 0, start: 0, end: 3 }, { patternIndex: 0, start: 3, end: 6 }]);
});
